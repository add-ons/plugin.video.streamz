...
check-addon: build
	...
	@unzip dist/plugin.video.streamz-*+matrix.1.zip -d ${TMPDIR}
...
release:
ifneq ($(release),)
	docker run -it --rm --env CHANGELOG_GITHUB_TOKEN=$(GH_TOKEN) -v "$(shell pwd)":/usr/local/src/your-app githubchangeloggenerator/github-changelog-generator -u add-ons -p plugin.video.streamz --no-issues --exclude-labels duplicate,question,invalid,wontfix,release,testing --future-release v$(release)
